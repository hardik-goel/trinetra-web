...
export async function request(base, path, opts = {}) {
  ...
  try {
    ...
    /* The browser says "Failed to fetch" for a sleeping host, a wrong URL,
       being offline AND a CORS rejection — deliberately identical wording,
       because telling them apart is what a cross-origin probe would be. It
       is also a fragment, so anything that appends a hint to it produces two
       sentences run together. Status 0 keeps "no response" distinguishable
       from any status the server actually sent. */
    throw new ApiError(0, null, "The backend could not be reached. It may be asleep, at a different URL, or refusing this origin.");
  }
