...
/** True when a target was computed and REJECTED for sitting closer than
    ...
    four places now: `exits`, `potential` (8563917 — sorting reads the
    potential block, so the flag has to be there too), the flat signal,
    and `levels` on a history record.
...
export function noRoomOf(x) {
  for (const v of [x?.exits?.noRoom, x?.potential?.noRoom, x?.noRoom, x?.levels?.noRoom]) {
