...
/* Every call goes through lib/api so the session cookie and the CSRF
   header are attached in one place. A local fetch here would compile,
   run, and be anonymous — which surfaces as a 401 that reads like a
   failed login rather than a missing option. */
const call = (base, path, { method = "GET", body, token } = {}) =>
  request(base, path, {
    method, body,
    ...
    headers: token ? { "X-Backup-Token": token } : undefined,
  });
...
/** True when a target was computed and REJECTED for sitting closer than
    ...
    The boolean is authoritative wherever it is sent, and it is sent in
    five places now: `exits`, `potential` (8563917 — sorting reads the
    potential block, so the flag has to be there too), the flat signal
    (which is also where 2e5641a's read-time guard puts it), `exitLevels`
    on a record written since that guard, and `levels` on any record.

    ABSENT IS NOT FALSE on a history record: it means the row predates
    the field. That case cannot be answered here — `noRoomWas()` below
    is the three-way read for stored rows.

    The last resort infers from shape, for payloads that predate the
    field being added to their endpoint (`/playbook/all` rows on Render
    today). A server-written riskRewardWarning with no target level
    anywhere cannot arise any other way: the warning is only written
    once a ratio has actually been computed. */
export function noRoomOf(x) {
  for (const v of [x?.exits?.noRoom, x?.potential?.noRoom, x?.noRoom,
                   x?.exitLevels?.noRoom, x?.levels?.noRoom]) {
    if (v === true) return true;
    if (v === false) return false;
  }
  const warning = x?.exits?.riskRewardWarning ?? x?.riskRewardWarning ?? x?.levels?.riskRewardWarning;
...
/** The three-way read for a stored signal: was a target rejected, was
    one sent, or does this row predate the field?

    Returns "rejected" | "sent" | "unknown". A track record that collapses
    the third into either of the first two is reporting a fact it does not
    have — the whole reason the column exists is to be checkable against
    the message that was actually received. */
export function noRoomWas(levels) {
  if (!levels) return "unknown";
  if (levels.noRoom === true) return "rejected";
  if (levels.noRoom === false) return "sent";
  // Pre-field rows: a warning with no exit is still unambiguous.
  if (levels.exit == null && levels.riskRewardWarning) return "rejected";
  return "unknown";
}
