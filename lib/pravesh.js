...
export function normalize(p) {
  ...
  return {
    ...
    counts,
    ipos,
    /* Engine schema 4+. False means the calendar answered but carried no dates, so
       nothing could be placed in a window: `counts` reads all zeros and `ipos` is
       empty for the OPPOSITE reason a genuinely quiet day is. Absent on older
       payloads, where the distinction did not exist — default to true, which is what
       every run before schema 4 was implicitly claiming. */
    calendarReadable: pick(root, "calendar_readable", "calendarReadable") !== false,
